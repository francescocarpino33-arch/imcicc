  ...
  return (
    <div className="pt-40 pb-32 px-6 md:px-12">
      <section className="mb-24 md:mb-32 max-w-5xl">
        ...
        <p className="mt-10 max-w-2xl text-lg md:text-xl text-primary-glow/70 leading-relaxed">
          ...
          {FILTERS.map((f) => {
            ...
            return (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`relative font-display text-sm tracking-[0.2em] font-light pb-2 transition-colors ${
        ...
        {filtered.map((p, i) => (
          ...
          >
            <Link to="/work/$slug" params={{ slug: p.slug }} className="group block">
              ...
              <div>
                  <h3 className="font-display text-2xl md:text-3xl font-light tracking-[-0.02em] group-hover:text-primary-glow transition-colors">
                    {p.title}
                  </h3>
